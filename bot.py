...
def get_top_tokens():
    ...
    try:
        # UWAGA: verify=False jest potencjalnie niebezpieczne.
        ...
        # ZMIANA: Używamy klucza 'channel_calls' do sortowania, który jest dostępny w API
        ...
        if sort_key not in data[0]:
            logging.error(f"Sort key '{sort_key}' not found in API response. Available keys: {data[0].keys() if data else 'None'}")
            return None # Nie można sortować, jeśli klucz nie istnieje
